...
////TODO
//Map SEIR::take_step(Disease& disease, analytics& a, Map& current_map) {
//	//NOT CORRECT RETURN
//	return current_map;
//}
...
Map SEIR::take_step(Disease& disease, analytics& a, Map& current_map) {
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			...
			else if (current_map.get_person_state(i, j) == 'E') {
				take_step_e(i, j,disease, nextMap, current_map);
			}
...
int SEIR::take_step_s(int y, int x, Disease& disease, Map& nextMap, Map& current_map) {
...
int SEIR::take_step_i(int i, int j, Map& nextMap, Map& current_map) {
